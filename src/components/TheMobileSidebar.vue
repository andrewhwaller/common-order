<template>
  ...
  <div v-if="!sidebarOpen && !splashScreen"
      class="md:hidden z-40 absolute bottom-0 left-0 mb-3 ml-2">
    <button
        @click="openSidebar()"
        ...
        <div class="relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-white">
          ...
          <div class="flex-1 flex flex-col overflow-y-auto">
            <nav class="flex-1 px-2 py-4 space-y-8 bg-white">
              <div class="space-y-1">
                <router-link v-for="item in navigation" :key="item.name" :to="item.href" :class="[isActive(item.href) ? 'bg-gray-100 text-gray-900 font-medium' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900', 'group flex items-center px-2 py-2 text-base rounded-md']" @click="closeSidebar()">
